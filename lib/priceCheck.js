  ...
  try {
    ...
    if (oldPrice !== newPrice) {
      ...
      if (shouldAlert) {
        ...
        // Try service-role admin lookup first (works when this is a service-role
        // client, e.g. called from the cron route). The admin namespace exists on
        // every client regardless of key type, so we must check the actual result,
        // not just whether the method exists.
        try {
          ...
          // Not a service-role client — admin API unavailable, fall through.
        }

        // Fall back to the current authenticated session (works when called from
        // an authenticated user route, e.g. the manual Check Now endpoint).
        if (!recipientEmail && supabase?.auth?.getUser) {
