...
import { SITE_NAME, EMAIL_FOOTER } from "@/lib/content";
...
export async function sendPriceDropAlert(
  ...
  try {
    ...
    const hasTarget = targetPrice !== null && targetPrice !== undefined;
    const subject = hasTarget
      ? `🎯 ${SITE_NAME}: reached your target price on ${product.name}`
      : `🎉 ${SITE_NAME}: price drop on ${product.name}`;

    const bannerTitle = hasTarget
      ? "🎯 Target Price Reached!"
      : "🎉 Price Drop Alert!";

    const calloutHtml = hasTarget
      ? `<div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 15px; margin: 20px 0; border-radius: 4px;">
           <p style="margin: 0; font-size: 14px; color: #92400e;">
             <strong>Reached your target price of ${product.currency} ${Number(targetPrice).toFixed(2)}!</strong>
           </p>
         </div>`
      : `<div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 15px; margin: 20px 0; border-radius: 4px;">
           <p style="margin: 0; font-size: 14px; color: #92400e;">
        ...
        <html>
          ...
          <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: linear-gradient(135deg, #FA5D19 0%, #FF8C42 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
              <h1 style="color: white; margin: 0; font-size: 28px;">${bannerTitle}</h1>
            </div>
